...
% all 3di files. Use files from 2009 to 2015.
d=dir([prefix,'archm*3di.nc']); 
id1=2387; % index for first day of 2009 
idn=2810; % index for last day of 2015
...
ilo=find(lon<=-46 & lon>=-54);
ila = find(lat <= -34.44 & lat >= -34.56)
start=[ilo(1) ila(1) 1 1];
count=[length(ilo) length(ila) 32 1];

% define big matrices
tem=nan*ones(32,length(ilo),idn-id1+1);
sal=tem;
%den=tem;
uu=tem;
vv=tem;
for ii=id1:idn % from 2009 to 2015
...
save hycom_34S yy mm dd tem sal  vv lat lon dep
