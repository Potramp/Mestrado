...
for i = 2:4;
	...
	hold on;
	contour([1:12],-dep_p,month_avg(:,:,i),[16 16],'linecolor','m','linewidth',2);colorbar;
...
for i = 1:4;
        ...
        hold on;
        contour([1:12],-dep,Tmean(:,:,i),[16 16],'linecolor','m','linewidth',2);colorbar;
	ylim([-1000 0]);
...
for j = 1:4;
	for i = 1:length(Tseries);
		if(sum(isnan(Tseries(:,i,j)) == 0 ));
			if(sum(diff(Tseries(:,i,j)) ~= 0) & sum(diff(Tseries(:,i,j)) ~= NaN));
...
for j = 2:4;
	for i = 1:ITsize(2);
		if(sum(isnan(ISAS_Tseries(:,i,j)) == 0) & sum(diff(ISAS_Tseries(:,i,j) ~= NaN)) & sum(diff(ISAS_Tseries(:,i,j) ~= 0)));
