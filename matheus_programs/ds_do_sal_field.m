%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is a version of Chris Meinen's do_sal_field.m script, with the
...
% I   - Replicate 3 times the monthly averaged residual of tau(p)* and
...
%%%% WHAT COMES AHEAD IS A COPY OF do_sal_field.m WITH 
...
predictedT=interp2(taurange,presrange,SMF_salinity,tau,pres);
 ...
 for j=1:length(errtaubin)
  ...
  %  if ~isempty(indx2)
    ...
    scattergridpct(i,j)=100*(mean(std(sal(indx(indx2)) - ...
                     predictedT(indx(indx2)))))/salrange;
    rmsgridpct(i,j)=100*(mean(rms(sal(indx(indx2)) - ...
                     predictedT(indx(indx2)))))/salrange;
    biasgridabs(i,j)=mean(sal(indx(indx2))) - ... 
                                mean(mean(SMF_salinity(gindx,gindx2)));
    scattergridabs(i,j)=mean(std(sal(indx(indx2)) - predictedT(indx(indx2))));
  rmsgridabs(i,j)=mean(rms(sal(indx(indx2)) - predictedT(indx(indx2))));
