...
yy = date_pies(:,1);
dd = date_pies(:,3);
% T_tilde_y FOR SEASONAL CORRECTION
load T_tilde_y
...
%%% Making T_tilde_y the size as SMF_temperature filling with NaNs.
junk = size(SMF_temperature);
junk2 = size(T_tilde_y);
junk = 0.*ones(junk(1),junk2(2));
junk2 = size(T_tilde_y);
junk(1:junk2,:) = T_tilde_y;
T_tilde_y = junk;
...
for j = 1:4;
	...
	for i = 1:length(tau_pies(:,j));
	       ...
	       if(tam(2)>0)
			...
			%%% an index variable for julian day, for 365 seasonal correction;
			indyear = datenum(yy(i) - yy(i), mm(i), dd(i));
			if(indyear <= 365)
				Tseries(:,i,j) = SMF_temperature(:,similar(indclose)) + T_tilde_y(:,indyear);
	       		else;
				Tseries(:,i,j) = SMF_temperature(:,similar(indclose)) + T_tilde_y(:,365);
			end;
	       else
