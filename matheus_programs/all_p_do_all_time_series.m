%%%% UNLIKE THE OTHER VERSION, THIS CALCULATES FOR ALL p reference levels
...
addpath ~/Dropbox/Mestrado/matheus_programs/

my_p = 100:20:360;
for w = 1:length(my_p)
        %%%% CHANGE FOLDER
        str = (['cd ~/Dropbox/Mestrado/matheus_programs/' num2str(my_p(w))]);
        eval(str);
	...
	% T_tilde FOR SEASONAL CORRECTION
        str = (['load ' num2str(my_p(w)) '/T_tilde;']);
        eval(str);
