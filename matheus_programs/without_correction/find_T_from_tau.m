% THIS CODE PLOTS A TIME SERIES FROM THE DATA IN SAM_PIES_data.txt
	...
	for i = 1:length(tau_pies(:,j));
	       ...
	       if(tam(2)>0)
			...
			%%% I need to make the seasonal correction for T as in Watts 2001.
	               ...
	               Tseries(:,i,j) = SMF_temperature(:,similar(indclose));
	       else
...
contourf(timeaxis(1:temsize(1)),-presrange(1:35),Tseries(1:35,1:temsize(1),4), 20);
xlabel('Data');
datetick; colorbar;
ylabel('p (dB)');
title('MEU')
...
contourf(timeaxis(1:temsize(1)),-presrange(1:35),tem(1:temsize(1),1:35)'-Tseries(1:35,1:temsize(1),4), 20);
