...
for j = 1:4;
        for i = 1:12
                ...
                % LOADING S VALUES FROM T_woa_PIES(j).asc
                % tem = tem(lon,lat,dep)
...
legend('PIES A','PIES B','PIES C','PIES D','Location','east')
xlabel('Percent Variance','fontsize',15);
...
ylabel('depth (m)','fontsize',15);
legend('PIES A','PIES B','PIES C','PIES D','Location','south')
subplot(1,2,2)
%title('VARIANCE A - m, B - k, C - b, D - g.')
hold on;
str = (['plot(var(T_woa_PIES' letra(i) ',0,2),-dep_woa, ''' cores(i) ''');']);
%str = (['plot(var(T_woa_PIES' letra(i) ',0,2)./mean(T_woa_PIES' letra(i) ',2),-dep_woa, ''' cores(i) ''');']);
eval(str);
set(gca,'YTick',[-1500:100:0]);
